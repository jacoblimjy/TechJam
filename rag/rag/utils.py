...
def get_few_shot_examples(
    ...
    try:
        ...
        # Read feedback and categorize by vote (latest first approach)
        positive_requests = {}  # request_id -> feedback_data
        negative_requests = {}  # request_id -> feedback_data
        ...
        # Early exit if no feedback available
        if not positive_requests and not negative_requests:
            return "" if format_as_text else []
        
        # Read classify log in reverse to get latest examples first
        # Process entries one by one instead of loading all into memory
        seen_rule_combinations = set()
        
        with open(classify_path, 'r', encoding='utf-8') as f:
            # Read all lines first (needed for reverse processing)
            lines = f.readlines()
        
        # Process from latest to oldest (reverse order) but stop early
        for line in reversed(lines):
            ...
            # Check for positive examples (prioritize latest, diverse rule combinations)
            if request_id in positive_requests and len(positive_examples) < max_positive:
                ...
                example = {
                    ...
                    "feedback_note": "GOOD classification (upvoted by user)",
            ...
            # Check for negative examples (prioritize latest, diverse rule combinations)
                ...
                example = {
                    ...
                    "rule_hits": log_entry.get("rule_hits", []),
                    "response": log_entry.get("response", {}),
                    "feedback_note": "AVOID - Poor classification (downvoted by user)",
                    "confidence": confidence,
                    "timestamp": log_entry.get("ts", "")
                }
